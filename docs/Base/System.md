...
构建：前端的构建其实只是狭义上的工程化，一般是代码校验、格式化、编译、压缩混淆和打包，以前前端项目常用的工具链`ESLint + Prettier + Babel + Terser + Webpack`，广义上的工程化还包括部署，运维，监控，日志等等一系列相关基建

至于`tree-shaking`、`code-splitting`、`HMR`、`source-map`这些是构建工具提供的功能

`build tool`和`bundler`的区别：构建工具可以自动执行构建前端代码的过程，，构建工具包括任务运行器`task runners`、打包器`bundlers`和编译器`compilers`

`task runners`：指自动化执行重复任务的工具（如测试、校验linting、格式化formatting、压缩minifying）

`bundlers`：将多个文件打包成一个文件，减少加载时间，提高性能

`compilers`：使开发人员能够用一种语言开发并且编译到另一种语言，例如用`TypeScript`写`JavaScript`代码
...
跨端：都是实现一个容器，给容器提供统一的`api`，这套`api`由不同平台各自去实现，保证功能一致，这样同一份代码能跑在不同平台的相同容器内，缺点：多了一层容器，性能比直接调用系统`api`会有所下降，当某项能力容器没有提供的时候，需要借助`bridge`进行扩展，例如`js`引擎的`JSbridge`，`jvm`的`jni`，至于编译成原生组件或内置浏览器或`WebView`去承载就由框架自己决定，现在移动端的跨端方案有基于`WebView`、渲染成原生组件（如`React Native`）和`Flutter`（自渲染引擎）
...
`CDN`：存储静态资源，在靠近用户的地方，建一个缓存服务器，把远端的内容复制一份放着，采用更多的缓存服务器（也称为CDN边缘节点），布放在用户访问相对集中的地区或网络中。当用户访问网站时，利用全局负载技术，将用户的访问指向距离最近的缓存服务器上，由缓存服务器响应用户请求

`Edge`：能够存储静态资源，缓存和执行代码，可以看成CDN的超集
...
5、阿里前端知识图谱：https://f2e.tech/

## 给自己的话

1、碰到一个新的工具，要以这样的思维去看待：明白它的概念，出现背景是什么，解决了什么痛点，多探究为什么，而不是关注如何用，避免成为API调用师
