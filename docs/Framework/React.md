...
`useEffect`是为了保持与外部系统同步，例如接口数据，访问DOM等，只涉及到`state`或`prop`等不应该放在`useEffect`，因为`prop`和`state`是`React`系统里的，外部系统是指非`React`系统的部分，例如后端接口数据和第三方库，而在实际开发中需要与第三方库同步的情况极少，一般`useEffect`只用于调接口，所以简单判断是否需要用`useEffect`的条件是是否需要调接口，具体见：[You Might Not Need an Effect](https://react.dev/learn/you-might-not-need-an-effect)
